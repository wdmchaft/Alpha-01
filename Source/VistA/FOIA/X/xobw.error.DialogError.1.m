 ;xobw.error.DialogError.1
 ;(C)InterSystems, generated for class xobw.error.DialogError.  Do NOT edit. 03/03/2011 09:29:08AM
 ;;00D81801BF4379EF;xobw.error.DialogError
 ;
...
%ClassName(fullname) public {
	Quit $select($get(fullname,0):"xobw.error.DialogError",1:"DialogError") }
%Construct(%this,dialogCode,errorMessage)
	Set $zobjval(,/*code*/1,0,3,1)="",$zobjval(,/*errorType*/2,0,3,2)="",$zobjval(,/*text*/3,0,3,3)="",$zobjval(,/*text*/4,0,3,3)=""
	Set $zobjmod(,3)=0,$zobjmods(,3)=0
	Quit ..%OnNew(.dialogCode,.errorMessage)
%ConstructClone(%this,deep=0,cloned,location) public {
	If $data(cloned(+%this)) Quit cloned(+%this)
	Set object=%this,%this=$zobjnew("xobw.error.DialogError"),cloned(+object)=%this,cloned(+object,0)=object Set $zobjmod(,0)=1
...
%Extends(isclass) public {
	Quit ''$listfind($listbuild("xobw.error.DialogError","xobw.error.AbstractError","%Library.RegisteredObject"),$s(isclass[".":isclass,$e(isclass)'="%":"User."_isclass,1:"%Library."_$e(isclass,2,*))) }
%GetParameter(paramname="") public {
	Quit $case(paramname,"ERRORTYPE":"DIALOG","PROPERTYVALIDATION":2,:"") }
%IsA(isclass) public {
	Quit ''$listfind($listbuild("xobw.error.DialogError","xobw.error.AbstractError","%Library.RegisteredObject"),$s(isclass[".":isclass,$e(isclass)'="%":"User."_isclass,1:"%Library."_$e(isclass,2,*))) }
...
%ObjectModified(%this) public {
	Quit ''$zobjmod(,0) }
%OnNew(%this,dialogCode,errorMessage) public {
	set $zobjval(,/*errorType*/2,0,3,2)="DIALOG"
	set $zobjval(,/*code*/1,0,3,1)=dialogCode
	set x=errorMessage
...
zforceError(dialogInfo) public {
	kill %XOBWERR,%objlasterror
	set %XOBWERR=dialogInfo
	set $ECODE=",UXOBW," }
...
ztextGetSwizzled(%this) public {
	...
	Set oref.ElementType="xobw.error.DialogError:text",oref.ElementClassType="datatype",oref.Owner=+%this,oref.Storage=3,oref.OrefStorage=4
