 ;xobw.error.SoapError.1
 ;(C)InterSystems, generated for class xobw.error.SoapError.  Do NOT edit. 03/03/2011 09:29:09AM
 ;;007A756C3761823A;xobw.error.SoapError
 ;
%BindExport(%this,dev,Seen,RegisterOref,AllowedDepth,AllowedCapacity) public {
	i $d(Seen(""_%this)) q 1
	Set Seen(""_%this)=%this
	s sc = 1
	s proporef=$zobjproperty(%this,"soapFault")
...
%ClassName(fullname) public {
	Quit $select($get(fullname,0):"xobw.error.SoapError",1:"SoapError") }
%Construct(%this,fault)
	Set $zobjval(,/*code*/1,0,3,1)="",$zobjval(,/*errorType*/2,0,3,2)="",$zobjval(,/*soapFault*/5,0,3,5)="",$zobjval(,/*soapFault*/6,0,3,5)="",$zobjval(,/*text*/3,0,3,3)="",$zobjval(,/*text*/4,0,3,3)=""
	Set $zobjmod(,3)=0,$zobjmods(,3)=0
	Quit ..%OnNew(.fault)
%ConstructClone(%this,deep=0,cloned,location) public {
	If $data(cloned(+%this)) Quit cloned(+%this)
	Set object=%this,%this=$zobjnew("xobw.error.SoapError"),cloned(+object)=%this,cloned(+object,0)=object Set $zobjmod(,0)=1
	Set $zobjval(,1)=$zobjval(object,1),$zobjval(,2)=$zobjval(object,2),$zobjval(,5)=$zobjval(object,5)
	Merge $zobjval(,3,0)=$zobjval(object,3,0),$zobjval(,4,0)=$zobjval(object,4,0) Set $zobjval(,3)="",$zobjval(,4)=""
	Set $zobjval(,6)=$zobjval(object,6)
	If deep>0 {
		If $isobject(object.soapFault) Set $zobjval(,6,0)=$zobjval(object,6,0).%ConstructClone(1,.cloned)
	}
	Quit %this }
...
%Extends(isclass) public {
	Quit ''$listfind($listbuild("xobw.error.SoapError","xobw.error.AbstractError","%Library.RegisteredObject"),$s(isclass[".":isclass,$e(isclass)'="%":"User."_isclass,1:"%Library."_$e(isclass,2,*))) }
%GetParameter(paramname="") public {
	Quit $case(paramname,"ERRORTYPE":"SOAP","PROPERTYVALIDATION":2,:"") }
%IsA(isclass) public {
	Quit ''$listfind($listbuild("xobw.error.SoapError","xobw.error.AbstractError","%Library.RegisteredObject"),$s(isclass[".":isclass,$e(isclass)'="%":"User."_isclass,1:"%Library."_$e(isclass,2,*))) }
...
%ObjectModified(%this) public {
	Quit ''$zobjmod(,0) }
%OnNew(%this,fault) public {
	set $zobjval(,/*errorType*/2,0,3,2)="SOAP"
	set $zobjval(,/*code*/1,0,3,1)="" ; normally this holds the Cache Object error code but this is a SOAP fault
	set $zobjval(,/*soapFault*/5,0,3,5)=fault
...
%AddToSaveSet(%this,depth=3,refresh=0,norecurseon="") public {
	...
	Set Poref=$zobjval(,6,0) If Poref'="" {
...
zisFaultClient(%this) public {
	quit ($P(..soapFault.faultcode,":",2)=$P("Client",":",2)) }
zisFaultMustUnderstand(%this) public {
	quit ($P(..soapFault.faultcode,":",2)=$P("MustUnderstand",":",2)) }
zisFaultServer(%this) public {
	quit ($P(..soapFault.faultcode,":",2)=$P("Server",":",2)) }
zisFaultVersionMismatch(%this) public {
	quit ($P(..soapFault.faultcode,":",2)=$P("VersionMismatch",":",2)) }
zsoapFaultGet(%this) public {
	quit $zobjval(,/*soapFault*/5,0,3,5) }
zsoapFaultTypeGet(%this) public {
	quit $select(..isFaultServer():"Server",..isFaultClient():"Client",..isFaultVersionMismatch():"Version Mismatch",..isFaultMustUnderstand():"Must Understand",1:"Unknown") }
...
zcodeNormalize(%val)
	Quit $e(%val,1,50)
zsoapFaultGetSwizzled(%this) public {
	Quit $zobjval(,/*soapFault*/6,0,3,5) }
zsoapFaultIsModified(%this) public {
	Quit $zobjmod(,5) }
zsoapFaultIsValid(value) public {
	Quit 1 }
zsoapFaultNewObject(%this) public {
	Set newobject=##class(%SOAP.Fault).%New("") If newobject="" Quit ""
	Set $zobjval(,/*soapFault*/6,0,3,5)=newobject, $zobjval(,/*soapFault*/5,0,3,5)="",$zobjmods(,6)=1
	Quit newobject }
zsoapFaultSet(%this,newvalue) public {
	If newvalue'="" { If '$isobject(newvalue) { Quit $$Error^%apiOBJ(5807,newvalue) } If $zobjval(,/*soapFault*/6,0,3,5)=newvalue { Quit 1 } }
	Set $zobjval(,/*soapFault*/6,0,3,5)=newvalue,$zobjval(,/*soapFault*/5,0,3,5)="",$zobjmods(,6)=1
	Quit 1 }
zsoapFaultSetModified(%this,newvalue) public {
...
ztextGetSwizzled(%this) public {
	...
	Set oref.ElementType="xobw.error.SoapError:text",oref.ElementClassType="datatype",oref.Owner=+%this,oref.Storage=3,oref.OrefStorage=4
