 ;xobw.error.ObjectError.1
 ;(C)InterSystems, generated for class xobw.error.ObjectError.  Do NOT edit. 03/03/2011 09:29:09AM
 ;;011F77BADC006610;xobw.error.ObjectError
 ;
...
%ClassName(fullname) public {
	Quit $select($get(fullname,0):"xobw.error.ObjectError",1:"ObjectError") }
%Construct(%this,statuscode)
	Set $zobjval(,/*code*/1,0,3,1)="",$zobjval(,/*dcode*/5,0,3,5)="",$zobjval(,/*domain*/6,0,3,6)="",$zobjval(,/*errorType*/2,0,3,2)="",$zobjval(,/*text*/3,0,3,3)="",$zobjval(,/*text*/4,0,3,3)=""
	Set $zobjmod(,3)=0,$zobjmods(,3)=0
	Quit ..%OnNew(.statuscode)
%ConstructClone(%this,deep=0,cloned,location) public {
	If $data(cloned(+%this)) Quit cloned(+%this)
	Set object=%this,%this=$zobjnew("xobw.error.ObjectError"),cloned(+object)=%this,cloned(+object,0)=object Set $zobjmod(,0)=1
	Set $zobjval(,1)=$zobjval(object,1),$zobjval(,5)=$zobjval(object,5),$zobjval(,6)=$zobjval(object,6)
	Merge $zobjval(,3,0)=$zobjval(object,3,0),$zobjval(,4,0)=$zobjval(object,4,0) Set $zobjval(,3)="",$zobjval(,4)=""
	Set $zobjval(,2)=$zobjval(object,2)
	Quit %this }
...
%Extends(isclass) public {
	Quit ''$listfind($listbuild("xobw.error.ObjectError","xobw.error.AbstractError","%Library.RegisteredObject"),$s(isclass[".":isclass,$e(isclass)'="%":"User."_isclass,1:"%Library."_$e(isclass,2,*))) }
%GetParameter(paramname="") public {
	Quit $case(paramname,"ERRORTYPE":"OBJECT","PROPERTYVALIDATION":2,:"") }
%IsA(isclass) public {
	Quit ''$listfind($listbuild("xobw.error.ObjectError","xobw.error.AbstractError","%Library.RegisteredObject"),$s(isclass[".":isclass,$e(isclass)'="%":"User."_isclass,1:"%Library."_$e(isclass,2,*))) }
...
%NormalizeObject(%this)
	...
	Set:$zobjval(,/*dcode*/5,0,3,5)'="" $zobjval(,/*dcode*/5,0,3,5)=(..dcodeNormalize($zobjval(,/*dcode*/5,0,3,5)))
	Set:$zobjval(,/*domain*/6,0,3,6)'="" $zobjval(,/*domain*/6,0,3,6)=(..domainNormalize($zobjval(,/*domain*/6,0,3,6)))
...
%ObjectModified(%this) public {
	Quit ''$zobjmod(,0) }
%OnNew(%this,statuscode) public {
	set $zobjval(,/*errorType*/2,0,3,2)="OBJECT"
	do $system.Status.DecomposeStatus(statuscode,.err)
	set $zobjval(,/*code*/1,0,3,1)=$GET(err(1,"code"),"<unknown>")
	set $zobjval(,/*dcode*/5,0,3,5)=$GET(err(1,"dcode"),"<unknown>")
	set $zobjval(,/*domain*/6,0,3,6)=$GET(err(1,"domain"),"<unknown>")
	set x=$get(err(1))
...
zdecompose(%this,error) public {
	...
	set error("dcode")=$zobjval(,/*dcode*/5,0,3,5)
	set error("domain")=$zobjval(,/*domain*/6,0,3,6)
...
zdisplay(%this) public {
	...
	write !," Error DCode: ",$zobjval(,/*dcode*/5,0,3,5)
	write !,"Error Domain: ",$zobjval(,/*domain*/6,0,3,6)
	write !,"  Error Text: "
	set I=""
...
ztextGetSwizzled(%this) public {
	...
	Set oref.ElementType="xobw.error.ObjectError:text",oref.ElementClassType="datatype",oref.Owner=+%this,oref.Storage=3,oref.OrefStorage=4
