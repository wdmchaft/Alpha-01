 ;xobw.error.HttpError.1
 ;(C)InterSystems, generated for class xobw.error.HttpError.  Do NOT edit. 03/03/2011 09:29:09AM
 ;;002E95218A2990EF;xobw.error.HttpError
 ;
...
%ClassName(fullname) public {
	Quit $select($get(fullname,0):"xobw.error.HttpError",1:"HttpError") }
%Construct(%this,httpResponse)
	Set $zobjval(,/*code*/1,0,3,1)="",$zobjval(,/*errorType*/2,0,3,2)="",$zobjval(,/*statusLine*/5,0,3,5)="",$zobjval(,/*text*/3,0,3,3)="",$zobjval(,/*text*/4,0,3,3)=""
	Set $zobjmod(,3)=0,$zobjmods(,3)=0
	Quit ..%OnNew(.httpResponse)
%ConstructClone(%this,deep=0,cloned,location) public {
	If $data(cloned(+%this)) Quit cloned(+%this)
	Set object=%this,%this=$zobjnew("xobw.error.HttpError"),cloned(+object)=%this,cloned(+object,0)=object Set $zobjmod(,0)=1
...
%Extends(isclass) public {
	Quit ''$listfind($listbuild("xobw.error.HttpError","xobw.error.AbstractError","%Library.RegisteredObject"),$s(isclass[".":isclass,$e(isclass)'="%":"User."_isclass,1:"%Library."_$e(isclass,2,*))) }
%GetParameter(paramname="") public {
	Quit $case(paramname,"ERRORTYPE":"HTTP","PROPERTYVALIDATION":2,:"") }
%IsA(isclass) public {
	Quit ''$listfind($listbuild("xobw.error.HttpError","xobw.error.AbstractError","%Library.RegisteredObject"),$s(isclass[".":isclass,$e(isclass)'="%":"User."_isclass,1:"%Library."_$e(isclass,2,*))) }
...
%NormalizeObject(%this)
	...
	Set:$zobjval(,/*statusLine*/5,0,3,5)'="" $zobjval(,/*statusLine*/5,0,3,5)=(..statusLineNormalize($zobjval(,/*statusLine*/5,0,3,5)))
	new data,key
...
%ObjectModified(%this) public {
	Quit ''$zobjmod(,0) }
%OnNew(%this,httpResponse) public {
	set $zobjval(,/*errorType*/2,0,3,2)="HTTP"
	set $zobjval(,/*code*/1,0,3,1)=httpResponse.StatusCode
	set $zobjval(,/*statusLine*/5,0,3,5)=httpResponse.StatusLine
	set len=70
...
zdecompose(%this,error) public {
	...
	set error("statusLine")=$zobjval(,/*statusLine*/5,0,3,5)
...
zdisplay(%this) public {
	...
	write !,"       HTTP Error Code: ",$zobjval(,/*code*/1,0,3,1)
	write !,"      HTTP Status Line: ",$zobjval(,/*statusLine*/5,0,3,5)
...
ztextGetSwizzled(%this) public {
	...
	Set oref.ElementType="xobw.error.HttpError:text",oref.ElementClassType="datatype",oref.Owner=+%this,oref.Storage=3,oref.OrefStorage=4
